...
public class XmlFileLogger implements IStoreWriter {
    ...
    private void fillStringBuffer(MethodCallPO pCurrentMethodCall) {
        ...
        mCurrentBuffer.append("\" ");
        if (ConfigurationHelper.getBoolean("log.parameter.defaultvalue", true)) { // On log tous les param�tres
            mCurrentBuffer.append("parameter=\"").append(pCurrentMethodCall.getParams()).append("\" ");
            if (pCurrentMethodCall.getThrowableClass() == null) { // Le retour de cette m�thode s'est bien pass�
                if (pCurrentMethodCall.getReturnValue() == null) { // M�thode de type 'void'
